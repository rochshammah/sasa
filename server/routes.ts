...
// Import all necessary schemas, including the newly created ones
import {
  insertJobSchema,
  insertMessageSchema,
  insertRatingSchema,
  createUserRequestSchema,
  updateJobStatusSchema, // <--- NEW IMPORT for Job Update validation
  updateProfileSchema,  // <--- NEW IMPORT for Profile Update validation
} from "@shared/schema";
...
        // Ensure categoryId is treated as a string for the query (matches Drizzle/DB type if applicable)
...
    } catch (error: any) {
      if (error instanceof ZodError) {
        return handleZodError(res, error); // Use consistent Zod error handling
      }
      res.status(500).json({ message: error.message });
    }
  });
